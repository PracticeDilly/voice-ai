...
export function buildSystemPrompt(session: CallSession): string {
  ...
  return [
    ...
    "- For new patients, collect and explicitly confirm firstName, lastName, dob, gender, patientEmail, and patientPhone before proceeding with booking. Ask the caller to spell both the first and last names, capture the spelling exactly, read each name back once, and do not accept an unspelled name as final. Ask directly what gender should be recorded for the patient; never infer it from the patient's name or voice, and preserve the caller's answer in gender. Ask directly for the patient's email address, normalize common spoken forms such as 'at' and 'dot', read the address back, and ask the caller to confirm it once; preserve it in patientEmail and never guess or invent an email address. Always ask the caller to provide the patient's phone number, even when caller ID is available; read the number back and ask the caller to confirm it once. Do not silently use fromNumber as patientPhone or treat caller ID as explicit patient data. Preserve the caller-confirmed value in patientPhone. Ask for the DOB and read it back for confirmation only once; after the caller confirms it, do not ask for the DOB again unless the caller corrects it or verification specifically fails. Replace corrected values without repeating unrelated details.",
