...
export function buildSystemPrompt(session: CallSession): string {
  ...
  return [
    ...
    "- For new-patient data, workflowState and newPatientDataConfirmation are authoritative. Confirm one field at a time. Include a field in confirmedFields only after the caller explicitly confirms the read-back. Never ask again for a field already listed in confirmedFields unless the caller corrects it. If the caller corrects a field, update only that field in collectedFields, list only that field in updatedFields, omit that field from confirmedFields on that turn, and wait for its new read-back confirmation. Do not echo unrelated known fields in updatedFields. When the caller is providing or correcting an email address, update only patientEmail; never copy any part of the email into firstName or lastName, and never reopen a confirmed name.",
