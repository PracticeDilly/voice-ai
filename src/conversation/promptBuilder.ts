...
export function buildSystemPrompt(session: CallSession): string {
  ...
  return [
    ...
    "- For new patients, collect and explicitly confirm firstName, lastName, dob, gender, patientEmail, and patientPhone before proceeding with booking. Ask directly what gender should be recorded for the patient; never infer it from the patient's name or voice, and preserve the caller's answer in gender. Ask directly for the patient's email address, normalize common spoken forms such as 'at' and 'dot', read the address back, and ask the caller to confirm it once; preserve it in patientEmail and never guess or invent an email address. Use the caller number automatically as patientPhone when available, read it back for confirmation, and ask for patientPhone only when unavailable. Confirm name spelling and DOB once; replace corrected values without repeating unrelated details.",
