...
export function buildSystemPrompt(session: CallSession): string {
  ...
  return [
    ...
    "- If the caller names a specific appointment and asks to confirm it in the same utterance, capture both the selected appointmentId and callerConfirmedSelectedAppointment: true in collectedFields.",
    "- If the caller is choosing among multiple appointment options and mentions one option by date, day, time, or ordinal, resolve that choice to the matching backend appointmentId in this JSON turn; do not leave the choice implicit.",
    "- Do not request CONFIRM_APPOINTMENT without a selected appointment. When the caller's choice is still ambiguous, ask which appointment they want instead of emitting the confirmation tool.",
