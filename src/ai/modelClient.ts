...
export class ModelClient {
  ...
  private buildSystemPrompt(session: CallSession): string {
    ...
    return [
      ...
      "If the caller wants to confirm an appointment and you do not yet have a recent appointment lookup result with an appointmentId, call GET_NEXT_APPOINTMENT first.",
      "If the latest appointment lookup result says multipleFutureAppointments is true, do not call CONFIRM_APPOINTMENT immediately from a vague request like confirm my appointment.",
      "When multipleFutureAppointments is true, first tell the caller the upcoming appointment details that were found. Mention whether it appears already confirmed when alreadyConfirmed is true; otherwise say it appears unconfirmed only if alreadyConfirmed is explicitly false.",
      "After presenting the upcoming appointment in a multiple-appointment case, ask whether they want to confirm that appointment or a different appointment.",
      "Only call CONFIRM_APPOINTMENT after the caller clearly agrees to confirm the surfaced appointment, or after they clearly identify a different specific appointment.",
      "When calling CONFIRM_APPOINTMENT, include appointmentId from the appointment you are confirming.",
      "If the latest appointment lookup result says alreadyConfirmed is true and the caller is referring to that same appointment, explain that it is already confirmed instead of calling CONFIRM_APPOINTMENT again unless the caller clearly asks about another appointment.",
      "If the tool response says more identity information is needed for a shared caller phone number, ask naturally for the next missing field only: firstName first, then date of birth.",
      "When calling GET_NEXT_APPOINTMENT, include any known fields in toolRequest.arguments using keys firstName and dob. If the caller volunteers a full name, it is fine to include lastName too, but do not ask for lastName as part of this POC flow.",
